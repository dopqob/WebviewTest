...
def create_report_file():
    if not os.path.exists(REPORTURL):
        os.makedirs(REPORTURL)
    file = REPORTURL + time.strftime(ISOTIMEFORMAT, time.localtime(time.time())) + "_" + 'report.html'
    return file


# 创建截图文件
def create_screenshot_file(img_name):
    day = time.strftime("%Y-%m-%d", time.localtime(time.time()))    # 创建日期子文件夹
    shot_path = SCREENSHOTURL + day
    shot_time = time.strftime(ISOTIMEFORMAT, time.localtime(time.time()))
    img_type = '.png'
    if not os.path.exists(shot_path):
        os.makedirs(shot_path)
    file = shot_path + "\\" + shot_time + "_" + img_name + img_type
    return file
